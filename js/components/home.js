// home.js
// v2026-05-07 - NOVÉ: emit 'clear-shift' pro reset z nastavení
//             - OPRAVA: lepší error handling v saveShift - chyba se zobrazí a stav se nesmaže
//             - NOVÉ: reaguje na event clear-shift z main.js (reset formuláře)
//             - nic jsem nesmazal
...
window.app.component('home-component', {
  ...
  methods: {
    ...
    // v2026-05-07: OPRAVA error handling
    // - duplikát (code 101) → zobrazí chybu, stav se nesmaže, uživatel vidí co se stalo
    // - jiná chyba → zobrazí chybu, stav se nesmaže
    // - úspěch (code 000) → smaže stav, OK
    async saveShift() {
      ...
      try {
        ...
        if (res.code === '000') {
          // Úspěch - smaž stav a hotovo
          const kmText = this.calculatedKm > 0 ? ` (${this.calculatedKm} km)` : '';
          this.$emit('message', `✓ Směna uložena${kmText}`);
          this.clearShiftState();
          this.$emit('reload');
        } else if (res.code === '101') {
          // v2026-05-07: duplikát - zobraz chybu, NESMAŽ stav
          // uživatel vidí že je to duplikát a může se rozhodnout co dělat
          this.$emit('message', '⚠️ Tato směna je již uložena. Pokud vidíte chybu, jděte do Nastavení → Smazat směnu.');
        } else {
          // v2026-05-07: jiná chyba - zobraz chybu, NESMAŽ stav
          this.$emit('message', '❌ Chyba při ukládání: ' + (res.error || 'Neznámá chyba'));
        }
      } catch (error) {
        // v2026-05-07: výjimka (offline, timeout) - zobraz chybu, NESMAŽ stav
    ...
    // v2026-05-07: clearShiftState je teď veřejná metoda
    // volá se při úspěšném uložení i z nastavení přes clear-shift event
    clearShiftState() {
